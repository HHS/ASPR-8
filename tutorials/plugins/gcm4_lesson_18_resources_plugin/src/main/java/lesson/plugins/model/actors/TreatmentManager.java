...
public class TreatmentManager {
	...
	private void assessHospitalization(PersonId personId) {
		...
		boolean treatedWithAntiViral = personPropertiesDataManager
				.getPersonPropertyValue(personId, PersonProperty.TREATED_WITH_ANTIVIRAL);
		...
		if (randomGenerator.nextDouble() < probabilityOfSuccess) {
			personPropertiesDataManager
				.setPersonPropertyValue(personId, PersonProperty.IMMUNE, true);
		} else {
			personPropertiesDataManager
				.setPersonPropertyValue(personId, PersonProperty.DEAD_IN_HOSPITAL, true);
		}
	...
	private void hospitalizePerson(PersonId personId) {
		...
		long availableHospitalBeds = resourcesDataManager
				.getRegionResourceLevel(regionId, Resource.HOSPITAL_BED);
		
		if (availableHospitalBeds > 0) {
			resourcesDataManager
				.transferResourceToPersonFromRegion(Resource.HOSPITAL_BED, personId, 1L);
			
			personPropertiesDataManager
				.setPersonPropertyValue(personId, PersonProperty.HOSPITALIZED, true);
	...
	private void treatWithAntiviral(PersonId personId) {
		...
		long regionResourceLevel = resourcesDataManager
				.getRegionResourceLevel(regionId, Resource.ANTI_VIRAL_MED);
		
		if (regionResourceLevel > 0) {
			resourcesDataManager
			.transferResourceToPersonFromRegion(Resource.ANTI_VIRAL_MED, personId, 1L);
			
			personPropertiesDataManager
			.setPersonPropertyValue(personId, PersonProperty.TREATED_WITH_ANTIVIRAL, true);
	...
	public void init(ActorContext actorContext) {	
		...
		StochasticsDataManager stochasticsDataManager = actorContext
				.getDataManager(StochasticsDataManager.class);
		...
		List<PersonId> susceptiblePeople = personPropertiesDataManager
				.getPeopleWithPropertyValue(PersonProperty.IMMUNE, false);
		for (PersonId personId : susceptiblePeople) {			
				double symptomOnsetTime = randomGenerator.nextDouble() 
						* maximumSymptomOnsetTime;
				personPropertiesDataManager
					.setPersonPropertyValue(personId, PersonProperty.INFECTED, true);
