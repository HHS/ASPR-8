 ...
 * <li>suppliers of consumers of {@linkplain AgentContext} for materials
 * producer initialization</li>
...
@Immutable
public final class MaterialsInitialData implements DataView {
	...
	public static class Builder {
		 ...
		 *             <li>{@linkplain MaterialsError#NULL_MATERIALS_PRODUCER_INITIAL_BEHAVIOR_SUPPLIER}
		 *             if the supplier is null</li>
		 */
		public Builder addMaterialsProducerId(final MaterialsProducerId materialsProducerId, final Supplier<Consumer<AgentContext>> supplier) {
			validateMaterialsProducerIdNotNull(materialsProducerId);
			validateComponentInitialBehaviorSupplierNotNull(supplier);
			data.materialsProducerIds.put(materialsProducerId, supplier);
			return this;
		}
		 ...
		 *             <li>{@linkplain MaterialsError#BATCH_STAGED_TO_DIFFERENT_OWNER} if a
		 *             batch is associated with a stage that is not owned by the
		 *             same materials producer as the batch</li>
		 */

		public MaterialsInitialData build() {
			try {
				validateData(data);
				return new MaterialsInitialData(data);
			} finally {
				data = new Data();
			}
		}
	...
	private static void validateComponentInitialBehaviorSupplierNotNull(final Supplier<Consumer<AgentContext>> supplier) {
		if (supplier == null) {
			throw new ContractException(MaterialsError.NULL_MATERIALS_PRODUCER_INITIAL_BEHAVIOR_SUPPLIER);
		}
	}
	...
	private static void validateData(final Data data) {
		...
		/*
		 * All batch property definitions must have default values since batches
		...
		for (final MaterialsProducerId materialsProducerId : data.materialsProducerPropertyValues.keySet()) {
			if (!data.materialsProducerIds.containsKey(materialsProducerId)) {
		...
		for (final MaterialsProducerPropertyId materialsProducerPropertyId : data.materialsProducerPropertyDefinitions.keySet()) {
			final PropertyDefinition propertyDefinition = data.materialsProducerPropertyDefinitions.get(materialsProducerPropertyId);
			if (!propertyDefinition.getDefaultValue().isPresent()) {
				for (final MaterialsProducerId materialsProducerId : data.materialsProducerIds.keySet()) {
	...
	private MaterialsInitialData(final Data data) {
		this.data = data;
	}
	...
	public <T extends MaterialsProducerId> Set<T> getMaterialsProducerIds() {
		final Set<T> result = new LinkedHashSet<>(data.materialsProducerIds.keySet().size());
		for (final MaterialsProducerId materialsProducerId : data.materialsProducerIds.keySet()) {
	...
	/**
	 * Returns the initial behavior of the given material producer
	 *
	 * @throws ContractException
	 ...
	 */
	public Consumer<AgentContext> getMaterialsProducerInitialBehavior(final MaterialsProducerId materialsProducerId) {
		validateMaterialsProducerExists(data, materialsProducerId);
		return data.materialsProducerIds.get(materialsProducerId).get();
	}
