...
public class TreeBitSetPeopleContainer implements PeopleContainer {
	 ...
	 * 
	 */
	public TreeBitSetPeopleContainer(PersonDataView personDataView) {
		...
		int capacity = personDataView.getPersonIdLimit();
	...
	private void grow(int pid) {
		...
		for (int i = 0; i < exclusizeMaxId; i++) {
			if (oldBitSet.get(i)) {
				safeAdd(personDataView.getBoxedPersonId(i));
			}
		}
	}
	...
	public PersonId getRandomPersonId(RandomGenerator randomGenerator) {
		...
		for (int i = bitSetStartIndex; i < bitSetStopIndex; i++) {
			if (bitSet.get(i)) {
				targetCount--;
				if (targetCount == 0) {
					return personDataView.getBoxedPersonId(i);
				}
			}
		}
		return null;
	...
	public List<PersonId> getPeople() {
		...
		for (int i = 0; i < n; i++) {
			if (bitSet.get(i)) {
				result.add(personDataView.getBoxedPersonId(i));
			}
		}
		return result;
	}
