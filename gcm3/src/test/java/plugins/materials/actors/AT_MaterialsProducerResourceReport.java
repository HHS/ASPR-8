		...
		for (TestMaterialsProducerId testMaterialsProducerId : producerIds) {
			...
			pluginBuilder.addTestActorPlan("actor", new TestActorPlan(actionTime++, (c) -> {
				...
				if(randomGenerator.nextBoolean()) {
					...
					expectedReportItems.add(getReportItemFromResourceId(c, testMaterialsProducerId, testResourceId, amount));	
				}else {
					long resourceLevel = materialsDataManager.getMaterialsProducerResourceLevel(testMaterialsProducerId, testResourceId);
					if(resourceLevel>0) {
					...
					materialsDataManager.transferResourceToRegion(testMaterialsProducerId, testResourceId, testRegionId, amount);
					expectedReportItems.add(getReportItemFromResourceId(c, testMaterialsProducerId, testResourceId, -amount));
					}
				}
			}));
